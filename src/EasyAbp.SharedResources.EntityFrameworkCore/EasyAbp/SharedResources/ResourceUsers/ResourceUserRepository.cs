...
namespace EasyAbp.SharedResources.ResourceUsers
{
    public class ResourceUserRepository : EfCoreRepository<SharedResourcesDbContext, ResourceUser, Guid>, IResourceUserRepository
    {
        ...
        public async Task<ResourceUser> FindAsync(Guid resourceId, Guid userId, CancellationToken cancellationToken = default)
        {
            return await GetQueryable().Where(x => x.UserId == userId && x.ResourceId == resourceId)
                .FirstOrDefaultAsync(cancellationToken);
        }

        public async Task<List<ResourceUser>> GetListByResourceIdAsync(Guid resourceId, CancellationToken cancellationToken = default)
        {
            return await GetQueryable().Where(x => x.ResourceId == resourceId).ToListAsync(cancellationToken);
        }

        public async Task<List<ResourceUser>> GetListByUserIdAsync(Guid userId, CancellationToken cancellationToken = default)
        {
